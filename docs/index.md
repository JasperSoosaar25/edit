...
title: MyFMHY :3
layout: home
description: My personal free media wiki ;3
hero:
  name: MyFMHY
  text: nya~ :3
  tagline: Your own super cute free media & tools collection ;333
  announcement:
...
features:
  - title: Adblocking / Privacy :3
    link: /privacy
    details: Learn how to block ads, trackers and other nasty things~ ;3
    icon: |
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ff9ede" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-shield-ellipsis"><path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/><path d="M8 12h.01"/><path d="M12 12h.01"/><path d="M16 12h.01"/><title>Ad-block / Privacy</title></svg>

  - title: Artificial Intelligence ;3
    link: /ai
    details: Explore the world of AI and machine learning~ nya~
    icon: |
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ff9ede" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-bot"><path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/><title>Artificial Intelligence</title></svg>

  - title: Streaming :3
    link: /video
    details: Stream, download, torrent and binge all your favourite movies and shows! ;3
    icon: |
  ...
  - title: Listening ;3
    link: /audio
    details: Stream, download and torrent songs, podcasts and more! nya~
    icon: |
  ...
  - title: Gaming :3
    link: /gaming
    details: Download and play all your favourite games or emulate some old but gold ones! ;3
    icon: |
  ...
  - title: Reading ;3
    link: /reading
    details: Whether you're a bookworm, otaku or comic book fan, you'll find your favourite pieces of literature here! nya~
    icon: |
  ...
  - title: Downloading :3
    link: /downloading
    details: Download all your favourite software, movies, shows, music, games and more! ;3
    icon: |
  ...
  - title: Torrenting ;3
    link: /torrenting
    details: Download your favourite media using the BitTorrent protocol~ :3
    icon: |
  ...
  - title: Educational :3
    link: /educational
    details: Educational content for all ages~ nya!
    icon: |
  ...
  - title: Android / iOS ;3
    link: /mobile
    details: All forms of content for Android and iOS~ :3
    icon: |
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#DAC9A6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-smartphone"><rect width="14" height="20" x="5" y="2" rx="2" ry="2"/><path d="M12 18h.01"/><title>Android / iOS</title></svg>

  - title: Linux / macOS :3
    link: /linux-macos
    details: The $HOME of Linux and macOS~ nya~
    icon: |
  ...
  - title: Non-English ;3
    link: /non-english
    details: Content in languages other than English~ :3
    icon: |
  ...
  - title: Miscellaneous :3
    link: /misc
    details: Various topics like food, travel, news, shopping, fun sites and more! ;3
    icon: |
...
onMounted(() => {
  ...
  const setKawaii = () => {
    ...
    images.forEach((img) => { img.src = '/logo-uwu.svg' })
  }

  const resetKawaii = () => {
    const images = document.querySelectorAll('.VPImage.image-src')
    images.forEach((img) => { img.src = '/test.png' })
  }

  if (kawaii === 'true') {
    try { localStorage.setItem('uwu', true) } catch (err) {}
    console.log('uwu mode enabled~ :3 Disable with "?uwu=false"')
    setKawaii()
  } else if (kawaii === 'false') {
    try { localStorage.removeItem('uwu') } catch (err) {}
    resetKawaii()
  ...
  const handleClick = () => {
    ...
    if (clickCount === 5) {
      ...
      if (isKawaii) {
        ...
        resetKawaii()
        console.log('uwu mode disabled. ;3')
      } else {
        localStorage.setItem('uwu', true)
        setKawaii()
        console.log('uwu mode enabled after 5 clicks~ nya!')
      }
      clickCount = 0
    }
  }
